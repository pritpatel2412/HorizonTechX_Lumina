...
export function PostCard({ post, queryKeyToInvalidate }: PostCardProps) {
  ...
  const handleLike = (e: React.MouseEvent, reaction?: string) => {
    ...
    // Optimistic update
    const isLiking = reaction ? true : !post.liked;
  ...
  const renderContent = (text: string) => {
    return text.split(/(#\w+)/g).map((part, i) => {
      if (part.startsWith('#')) {
        return <Link key={i} href={`/explore?tag=${encodeURIComponent(part.substring(1))}`} className="text-primary hover:underline cursor-pointer">{part}</Link>;
      }
      return part;
    });
  };
  ...
  return (
    <>
      <div className="lumina-card p-5 mb-4 animate-in fade-in slide-in-from-bottom-2">
        ...
        {isMoment ? (
          <div className="flex gap-2 mb-4 h-[300px] sm:h-[400px] rounded-xl overflow-hidden p-1 bg-gradient-to-br from-primary to-secondary">
            <div className="flex-1 h-full rounded-l-lg overflow-hidden bg-black cursor-pointer">
              <img src={post.imageUrl!} alt="Moment 1" className="w-full h-full object-cover hover:scale-105 transition-transform duration-500" onDoubleClick={() => setSpotlightImage(post.imageUrl!)} />
            </div>
            <div className="flex-1 h-full rounded-r-lg overflow-hidden bg-black cursor-pointer">
              <img src={post.imageUrl2!} alt="Moment 2" className="w-full h-full object-cover hover:scale-105 transition-transform duration-500" onDoubleClick={() => setSpotlightImage(post.imageUrl2!)} />
            </div>
          </div>
        ) : post.imageUrl ? (
          <div className="mb-4 rounded-xl overflow-hidden bg-black max-h-[500px] cursor-pointer border border-white/5">
            <img src={post.imageUrl} alt="Post media" className="w-full object-contain max-h-[500px] hover:scale-[1.02] transition-transform duration-500" onDoubleClick={() => setSpotlightImage(post.imageUrl!)} />
          </div>
        ) : null}
        ...
        <div className="flex items-center justify-between pt-2">
              ...
              >
                <Heart 
                  className={cn(
                    ...
                    post.liked && "fill-secondary text-secondary scale-110",
                    "group-active:scale-125 group-active:fill-secondary group-active:text-secondary"
                  )} 
                />
                <span className={cn("text-sm font-medium", post.liked && "text-secondary")}>{post.likeCount}</span>
              </button>
          ...
          <button 
            className={cn("text-muted-foreground transition-colors hover:text-white", post.saved && "text-primary")}
            onClick={handleSave}
          >
            <Bookmark className={cn("w-5 h-5", post.saved && "fill-primary text-primary")} />
          </button>
        </div>
          ...
          <div className="flex items-center gap-2 mt-3 pt-3 border-t border-white/5">
            <div className="flex -space-x-1">
              {post.topReactions.slice(0,3).map((reaction, i) => (
                <div key={i} className="w-6 h-6 rounded-full bg-surface-elevated flex items-center justify-center text-xs border border-surface z-10" style={{ zIndex: 10 - i }}>
                  {reaction.reaction}
                </div>
              ))}
            </div>
      ...
      {spotlightImage && (
        <div className="fixed inset-0 z-[100] bg-black/90 flex items-center justify-center p-4" onClick={() => setSpotlightImage(null)}>
          <button 
          ...
          <img src={spotlightImage} alt="Spotlight" className="max-w-full max-h-full object-contain animate-in zoom-in-95 duration-200" />
        </div>
      )}
    </>
  );
